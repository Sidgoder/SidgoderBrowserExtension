(function () {
  ...
  function customEngineById(engineId) {
    ...
    return engine ? Object.assign({ id: CUSTOM_PREFIX + engine.id }, engine) : null;
  }
  ...
  function engineOptions() {
    ...
    state.customEngines.forEach(function (engine) {
      options.push(Object.assign({ id: CUSTOM_PREFIX + engine.id }, engine));
    });
  ...
  function applyState() {
    ...
    el.engineSelect.value = engines[state.engine] ? state.engine : defaults.engine;
  ...
  function bindEvents() {
    ...
    el.engineList.addEventListener("click", function (event) {
      var button = event.target.closest("button[data-engine]");
